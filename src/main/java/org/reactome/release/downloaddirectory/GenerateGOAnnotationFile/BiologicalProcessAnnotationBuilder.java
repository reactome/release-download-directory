...
public class BiologicalProcessAnnotationBuilder {
    ...
    public static List<String> processBiologicalFunctions(GKInstance reactionInst) throws Exception {
        ...
        Collection<GKInstance> catalystInstances = reactionInst.getAttributeValuesList(
     ...
     * @param proteinInstances -- Set of GKInstances, these can be all catalyst proteins or, if no catalyst exists
     * in the reaction, all reaction proteins.
     * @param reactionInst -- GKInstance, parent reaction instance.
     * @throws Exception -- MySQLAdaptor exception.
     */
    private static List<String> processProteins(Set<GKInstance> proteinInstances, GKInstance reactionInst)
        throws Exception {
     ...
     * @param reactionInst -- GKInstance, parent reaction instance.
     * @param taxonIdentifier -- String, CrossReference ID of protein's species. Example: 9606 (Human Species
     * crossReference identifier from NCBI)
     * @throws Exception -- MySQLAdaptor exception.
     */
    private static List<String> getGOBiologicalProcessLine(GKInstance referenceEntityInst, GKInstance reactionInst,
                                                           String taxonIdentifier) throws Exception {
        List<String> goaLines = new ArrayList<>();
     ...
     * @param eventInst -- GKInstance, Can be the original reaction instance, or, if it had no Biological Process
     ...
     * @throws Exception -- MySQLAdaptor exception.
     */
    private static List<Map<String, String>> getGOBiologicalProcessAccessions(GKInstance eventInst, int recursion)
        throws Exception {

        List<Map<String, String>> goBiologicalProcessAccessions = new ArrayList<>();
        if (recursion <= MAX_RECURSION_LEVEL) {
            Collection<GKInstance> goBiologicalProcessInstances = eventInst.getAttributeValuesList(
            ...
            if (!goBiologicalProcessInstances.isEmpty()) {
                for (GKInstance goBiologicalProcessInst : goBiologicalProcessInstances) {
                    if (!GOAGeneratorUtilities.isProteinBindingAnnotation(goBiologicalProcessInst)) {
                        ...
                        String reactomeIdentifier =
                            REACTOME_IDENTIFIER_PREFIX + GOAGeneratorUtilities.getStableIdentifierIdentifier(eventInst);
                ...
                Collection<GKInstance> hasEventReferralInstances =
                    (Collection<GKInstance>) eventInst.getReferers(ReactomeJavaConstants.hasEvent);
     ...
     * @param goBiologicalProcessInst -- GKInstance, a GO Biological Process Instance
     * @return -- The accession of the GO Biological Process Instance with the "GO:" prefix
     * @throws Exception -- MySQLAdaptor exception
     */
    private static String getGOAccessionWithPrefix(GKInstance goBiologicalProcessInst) throws Exception {
        return GO_IDENTIFIER_PREFIX +
            goBiologicalProcessInst.getAttributeValue(ReactomeJavaConstants.accession).toString();
    }
